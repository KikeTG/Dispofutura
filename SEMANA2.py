...
# Última modificación: 2024-02-08 14:28:30
...
df_transito_original = pd.read_excel(ruta_destino_stock3, sheet_name='Hoja1')
...
def asignar_semana_al_mes(fecha):
    ...
    primer_dia_del_mes = fecha.replace(day=8)
...
# Establecer la fecha de referencia en el día 8 del mes actual
fecha_referencia = datetime(fecha_actual.year, fecha_actual.month, 8)

# Calcular el número de semana del año para el día 8 del mes actual
numero_semana_inicio_mes = fecha_referencia.isocalendar()[1]

# Encontrar el número de semana del año actual
numero_semana_anio_actual = fecha_actual.isocalendar()[1]

# Crear el nombre de la columna de la semana actual
nombre_columna_semana = f"Sem {numero_semana_inicio_mes} ({fecha_actual.strftime('%y')})"

# Agregar la nueva columna con el nombre de la semana calculada
merged_df2[nombre_columna_semana] = np.nan

# Convertir las columnas relevantes a numéricas y llenar los valores faltantes con cero
...
# Imprimir el nombre de la columna de la semana actual
...
semana_siguiente = numero_semana_anio_actual + 1
