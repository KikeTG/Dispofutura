...
# Última modificación: 2024-01-29 15:44:18
...
ruta_base = os.path.join(user_home_dir, "Inchcape", "Planificación y Compras Chile - Documentos", "Planificación y Compras Maestros", "2023")

nombre_carpeta = f"2023-{mes_anterior_numero}"
...
archivo_stock_tiendas3 = next((f for f in os.listdir(ruta_carpeta_seleccionada) if "onsolidado" in f and 'emana' in f), None)
...
df_transito_original = pd.read_excel(ruta_origen_stock3, sheet_name='Hoja1')
...
# Primero'Semana_Año'.
df_transito_original['Semana_Año'] = df_transito_original.apply(lambda x: str(x['Semanas']) + '-' + str(x['Año']), axis=1)
...
# Suponiendo que merged_df2 es un DataFrame existente.

# Obtener la fecha actual
fecha_actual = datetime.now()

# Establecer la fecha de referencia en el día 1 del mes actual
fecha_referencia = fecha_actual.replace(day=1)

# Encontrar el número de semana del año para el día 1 del mes
numero_semana_inicio_mes = fecha_referencia.isocalendar()[1]

# Calcular la semana actual como la diferencia entre la fecha actual y la fecha de referencia
diferencia_dias = (fecha_actual - fecha_referencia).days
semana_actual = ((diferencia_dias // 7) + 1) 

# Ajustar si la fecha de referencia no es la semana 1 del mes
if fecha_referencia.weekday() > 3:  # Si el día 1 no es jueves o antes, no está en la semana 1 según ISO 8601
    semana_actual += 1

nombre_columna_semana = f"Sem {semana_actual} ({fecha_actual.strftime('%y')})"
